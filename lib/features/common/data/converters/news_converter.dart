...
/// Конвертер из [New] (объект базы данных) в [NewsItemEntity].
typedef INewsDbToEntityConverter = Converter<NewsItemEntity, NewsItemDb>;

/// Реализация [INewsDbToEntityConverter].
final class NewsDbToEntityConverter extends INewsDbToEntityConverter {
  const NewsDbToEntityConverter();

  @override
  NewsItemEntity convert(NewsItemDb input) {
    return NewsItemEntity(
      id: input.newsId,
      type: input.type,
      sectionId: input.sectionId,
      ...
      pillarId: input.pillarId,
      pillarName: input.pillarName,
      fields: NewsFieldsEntity.fromDb(input),
    );
  }
}
